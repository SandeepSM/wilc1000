				       ...
				       struct drv_handler *hif_drv_handler)
{
	...
	currbyte = buffer;
	*currbyte = driver_handler_id & 0x000000FF;
	currbyte++;
	*currbyte = (u32)0 & 0x000000FF;
	currbyte++;
	*currbyte = (u32)0 & 0x000000FF;
	currbyte++;
	*currbyte =(u32)0 & 0x000000FF;
				...
				struct rf_info *pstrStatistics)
{
	...
	if (result)
		netdev_err(vif->ndev, "Failed to send scan parameters II\n");
...
int wilc_get_statistics(struct wilc_vif *vif, struct rf_info *stats)
{
	...
	/*The bus to WILC does not like to be bothered
	 while connect is ongoing, try later*/
