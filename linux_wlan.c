				...
				unsigned long event, void *ptr)
{
	...
	case NETDEV_UP:
		...
		if (wilc_enable_ps && (ip_addr_buf[0] != 169)){
			wait_for_completion(&vif->wilc->wait_for_ip);
...
void wilc_wlan_deinitialize(struct net_device *dev)
{
	...
	if (wl->initialized)	{
		...
		netdev_dbg(dev, "wilc deinitialization Done\n");
	} else {
		netdev_dbg(dev, "wilc is not initialized\n");
	}
}
...
static int wlan_init_locks(struct net_device *dev)
{
	...
	init_completion(&wl->wait_for_ip);
